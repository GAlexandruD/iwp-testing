...
import { setUserGlobal, setTryingToLogin } from "../redux/slices/userSlice";
...
const Login = () => {
  ...
  const tryingToLogin = useSelector((state) => state.user.tryingToLogin);
  const userGlobal = useSelector((state) => state.user.value);

  const handleOr = () => setSignup(!signup);
  ...
  const handleSignin = () => {
    signInWithEmailAndPassword(
      ...
      .then((userCredential) => {
        ...
        dispatch(setUserGlobal({ accessToken, displayName, email, uid }));
      })
      .catch((error) => {
  ...
  const handleSignup = () => {
    createUserWithEmailAndPassword(
      ...
      .then((userCredential) => {
        ...
        // ...
        dispatch(setTryingToLogin(true));
      })
      .catch((error) => {
        const errorCode = error.code;
        const errorMessage = error.message;
    ...
    if (!userGlobal && tryingToLogin) {
      getRedirectResult(auth)
        .then((result) => {
          ...
          console.log(
            "I have a bunch of things (user, token, credential): ",
            user
          );
          dispatch(setTryingToLogin(false));
          const { accessToken, displayName, email, uid } = user;
          dispatch(setUserGlobal({ accessToken, displayName, email, uid }));
        })
        .catch((error) => {
  ...
  return (
    <div className="m-8 flex w-full max-w-sm mx-auto overflow-hidden bg-white rounded-lg shadow-lg dark:bg-gray-800 lg:max-w-4xl">
      <ToastContainer
        position="top-center"
        autoClose={5000}
        ...
        <button
          ...
          onClick={() => {
            ...
            signOut(auth)
              ...
              .catch((error) => {
                // An error happened.
                toast("There was an error signin you out");
              });
          }}
        >
          LOGOUT
